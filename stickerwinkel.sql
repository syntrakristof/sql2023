...
(
    ...
    created        datetime       null,
    updated        datetime       null,
    constraint fk_colortype_fk
        foreign key (color_id) references colors (id),
    constraint fk_stickertype_fk_fk
        foreign key (stickertype_id) references sticker_types (id)
);
...
(
    ...
    price      decimal(10, 2) not null,
    constraint FK_sales_detail_stickers
...
select `s`.`price` AS `price`, `c`.`name` AS `name`, sum(`d`.`quantity`) AS `quantity`
...
select `st`.`name` AS `productname`, sum(`d`.`quantity`) AS `quantity`, sum(`d`.`price`) AS `totalprice`
